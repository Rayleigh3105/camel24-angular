...
export class OrderModalComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subs.push(this.$dashboard.getOrdersForKundenNummer(kundenNummertoSearchWith, null).subscribe(data => {
   ...
   */
  onRowSelect(event) {
    this.order = OrderModalComponent.cloneOrder(event.data);
   ...
   */
  static cloneOrder(cloneOrder): any {
    let order = {};
   ...
   */
  downloadPdf() {
    this.subs.push(this.$dashboard.downloadPdf(this.order.identificationNumber).subscribe(blob => {
      if (blob) {
   ...
   */
  public filterOrders() {
    this.subs.push(this.$dashboard.getOrdersForKundenNummer(this.currentKundenNummer, this.filterIdent).subscribe(data => {
      this.countData = data.length;
    }));
  }
}
